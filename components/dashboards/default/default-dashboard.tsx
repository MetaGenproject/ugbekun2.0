...
export function DefaultDashboard({ user, roleName }: DashboardProps) {
  const stats = [
    { label: 'Platform Status', value: 'Authenticated', icon: Shield, color: 'text-emerald-600 bg-emerald-50 border-emerald-100' },
    { label: 'User Role Level', value: roleName || 'Staff Member', icon: UserIcon, color: 'text-blue-600 bg-blue-50 border-blue-100' }
  ]

  const activities = [
    { desc: 'Logged into the modernized Ugbekun 2.0 workspace portal', time: 'Just now' }
  ]
      ...
      <div className="relative rounded-2xl border border-slate-200/80 bg-white p-6 md:p-8 shadow-sm overflow-hidden">
        ...
        <div className="relative z-10 space-y-2.5">
          ...
          <p className="text-slate-500 text-sm max-w-xl font-medium">
            Active school administration and operational control panel.
          </p>
        </div>
      </div>
