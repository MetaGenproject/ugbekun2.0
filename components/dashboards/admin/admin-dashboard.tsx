...
export function AdminDashboard({ user }: DashboardProps) {
  const stats = [
    { label: 'Total Students Enrolled', value: '2,450 Students', icon: Users, color: 'text-blue-600 bg-blue-50 border-blue-100' },
    { label: 'Active Academic Staff', value: '156 Teachers', icon: GraduationCap, color: 'text-emerald-600 bg-emerald-50 border-emerald-100' },
    { label: 'Today\'s Student Attendance', value: '94.2%', icon: CheckSquare, color: 'text-violet-600 bg-violet-50 border-violet-100' },
    { label: 'Tuition Fees Collected', value: '$240,000', icon: DollarSign, color: 'text-amber-600 bg-amber-50 border-amber-100' }
  ]

  const activities = [
    { desc: 'Teacher credential profile generated for Mrs. Sarah Jenkins', time: '10 mins ago' },
    { desc: 'Grade reports published for Q3 academic evaluations', time: '1 hour ago' },
    { desc: 'Tuition fees balance statement issued to parents', time: '3 hours ago' }
  ]
      ...
      <div className="relative rounded-2xl border border-slate-200/80 bg-white p-6 md:p-8 shadow-sm overflow-hidden">
        ...
        <div className="relative z-10 space-y-2.5">
          ...
          <p className="text-slate-500 text-sm max-w-xl font-medium">
            Role 2 · Branch-level controls — registrations, accounting, staff and curriculum tools.
          </p>
        </div>
      </div>
